...
const ProducerProfile: React.FC = () => {
    ...
    const followedArtists = useMemo(() => curArtists.filter(a => (producer.following?.artists || []).includes(a.id)), [curArtists, producer.following]);
    const followedEngineers = useMemo(() => curEngineers.filter(e => (producer.following?.engineers || []).includes(e.id)), [curEngineers, producer.following]);
    const followedStoodioz = useMemo(() => curStoodioz.filter(s => (producer.following?.stoodioz || []).includes(s.id)), [curStoodioz, producer.following]);
    const followedProducers = useMemo(() => curProducers.filter(p => (producer.following?.producers || []).includes(p.id)), [curProducers, producer.following]);
            ...
            <div className="max-w-4xl mx-auto space-y-12">
                <div className="p-8 cardSurface">
                    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-8">
                        {/* FIX: Corrected property name from 'imageUrl' to 'image_url' */}
                        <img src={getProfileImageUrl(producer)} alt={producer.name} className="w-32 h-32 sm:w-40 sm:h-40 rounded-full object-cover border-4 border-zinc-700 flex-shrink-0" />
