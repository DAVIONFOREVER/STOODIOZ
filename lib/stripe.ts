...
const getStripe = (): Promise<any> => {
    if (!stripePromise) {
        ...
        const publishableKey =
            envKey ||
            "pk_test_51SLl4UPQdrRi1Fr9g8FUPeKDK45boiPQmPCdpE9FGINBOyIUNsTMGr3DXNquPkgsPyx0Xr0I17szjcYPV7BRsn0e0040prQfo1";
        if (!publishableKey || !publishableKey.startsWith('pk_')) {
            console.error("Stripe publishable key is missing or invalid. Please set VITE_STRIPE_PUBLISHABLE_KEY.");
            return Promise.reject("Stripe key is not configured.");
        }
        if (!envKey) {
            console.warn("Using fallback Stripe test publishable key. Set VITE_STRIPE_PUBLISHABLE_KEY for production.");
        }
