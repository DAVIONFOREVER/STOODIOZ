...
const EngineerProfile: React.FC = () => {
    ...
    const allUsers = useMemo(() => [...curArtists, ...curEngineers, ...curStoodioz, ...curProducers], [curArtists, curEngineers, curStoodioz, curProducers]);
    const followers = useMemo(() => allUsers.filter(u => (engineer.follower_ids || []).includes(u.id)), [allUsers, engineer.follower_ids]);

    const followedArtists = useMemo(() => curArtists.filter(a => (engineer.following?.artists || []).includes(a.id)), [curArtists, engineer.following]);
    const followedEngineers = useMemo(() => curEngineers.filter(e => (engineer.following?.engineers || []).includes(e.id)), [curEngineers, engineer.following]);
    const followedStoodioz = useMemo(() => curStoodioz.filter(s => (engineer.following?.stoodioz || []).includes(s.id)), [curStoodioz, engineer.following]);
    const followedProducers = useMemo(() => curProducers.filter(p => (engineer.following?.producers || []).includes(p.id)), [curProducers, engineer.following]);
            ...
            <div className="max-w-4xl mx-auto space-y-12">
                <div className="p-8 cardSurface">
                    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-8">
                        {/* FIX: Corrected property name from 'imageUrl' to 'image_url' */}
                        <img src={getProfileImageUrl(engineer)} alt={engineer.name} className="w-32 h-32 sm:w-40 sm:h-40 rounded-full object-cover border-4 border-zinc-700 flex-shrink-0" />
