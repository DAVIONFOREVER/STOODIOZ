...
const ClaimConfirmScreen: React.FC = () => {
    ...
    useEffect(() => {
        const storedToken = sessionStorage.getItem('pending_claim_token') || localStorage.getItem('pending_claim_token');
        if (!storedToken) {
    ...
    const handleClaim = async () => {
        ...
        setLoading(true);
        try {
            const { role } = await claimProfileByToken(token, currentUser.id);
            sessionStorage.removeItem('pending_claim_token');
            localStorage.removeItem('pending_claim_token');
            alert(`Success! You have joined ${details?.labelName || 'the label'} roster. You now have a verified badge.`);
            if (role === 'ENGINEER') navigate(AppView.ENGINEER_DASHBOARD);
            else if (role === 'PRODUCER') navigate(AppView.PRODUCER_DASHBOARD);
            else navigate(AppView.ARTIST_DASHBOARD); 
        } catch (error: any) {
