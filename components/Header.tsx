...
const Header: React.FC<HeaderProps> = (props) => {
  ...
  const { userRole, notifications, artists, engineers, producers, stoodioz, currentUser, history, historyIndex } =
    useAppState();
  ...
  const handleLogoClick = () => {
    ...
    if (setupViews.includes(currentView)) {
      if (!window.confirm('You are currently setting up your profile. If you leave now, your progress will be lost. Continue?')) {
        return;
      }
    }

    if (userRole) onNavigate(AppView.THE_STAGE);
        ...
        <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
              ...
              {userRole ? (
                <div className="flex items-center space-x-1">
                  ...
                  <div ref={panelRef} className="border-l border-zinc-700 ml-2 pl-2 flex items-center gap-1 flex-shrink-0 relative">
                    <button onClick={() => onNavigate(AppView.INBOX)} className={`${navLinkClasses} flex items-center`} aria-label="Inbox">
                    ...
                    <button onClick={onLogout} className={`${navLinkClasses} flex items-center gap-1.5`} aria-label="Logout">
            ...
            <div className="lg:hidden flex items-center">
              <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 text-slate-300 hover:text-orange-400" aria-label="Open menu">
        ...
        <div className="fixed inset-0 z-[60] lg:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-[320px] max-w-[85vw] bg-zinc-950 border-l border-zinc-800 shadow-xl flex flex-col">
            <div className="flex items-center justify-between px-4 h-16 border-b border-zinc-800">
              ...
              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 text-slate-300 hover:text-orange-400" aria-label="Close menu">
            ...
            <div className="p-3 flex flex-col gap-2 overflow-y-auto">
              {!userRole ? (
                <>
                  <button onClick={() => navAndCloseMobile(AppView.SUBSCRIPTION_PLANS)} className="w-full text-left px-3 py-2 rounded-lg bg-zinc-900 text-slate-200">
                    Plans
                  </button>
                  <button onClick={() => navAndCloseMobile(AppView.STOODIO_LIST)} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-900 text-slate-200">
                    Find Stoodioz
                  </button>
                  <button onClick={() => navAndCloseMobile(AppView.ENGINEER_LIST)} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-900 text-slate-200">
                    Find Engineers
                  </button>
                  <button onClick={() => navAndCloseMobile(AppView.PRODUCER_LIST)} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-900 text-slate-200">
                    Find Producers
                  </button>
                  <button onClick={() => navAndCloseMobile(AppView.ARTIST_LIST)} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-900 text-slate-200">
                    Find Artists
                  </button>

                  <div className="h-px bg-zinc-800 my-2" />

                  <button onClick={() => navAndCloseMobile(AppView.LOGIN)} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-900 text-slate-200">
                    Login
                  </button>
                  <button
                ...
                <>
                  <button onClick={() => navAndCloseMobile(AppView.THE_STAGE)} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-900 text-slate-200">
                    The Stage
                  </button>
                  <button onClick={() => navAndCloseMobile(AppView.INBOX)} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-900 text-slate-200">
                    Inbox
                  </button>
                  <button onClick={() => navAndCloseMobile(AppView.ASSET_VAULT)} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-900 text-slate-200">
                    Vault
                  </button>
                  <button onClick={() => navAndCloseMobile(AppView.MASTER_CALENDAR)} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-900 text-slate-200">
                    Calendar
                  </button>

                  <button onClick={handleViewProfile} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-900 text-slate-200">
                    Profile
                  </button>
                  <button onClick={handleDashboardNavigate} className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-900 text-slate-200">
                    Dashboard
                  </button>
