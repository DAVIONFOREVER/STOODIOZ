...
const RankedUserCard: React.FC<RankedUserCardProps> = ({ profile, rank, isSpotlight = false, onSelectProfile }) => {
    ...
    if (isSpotlight) {
        return (
            <button 
                ...
                className="w-full bg-gradient-to-br from-black/50 to-zinc-900/50 backdrop-blur-md rounded-2xl p-6 border-2 border-orange-500/50 shadow-[0_0_30px_rgba(249,115,22,0.2)] flex flex-col md:flex-row items-center gap-6 text-left transition-all duration-300 hover:border-orange-500/80 hover:shadow-[0_0_40px_rgba(249,115,22,0.3)] hover:scale-[1.02]"
            >
                ...
                <div>
                    <h3 className="text-3xl font-bold text-orange-400 text-glow">{profile.name}</h3>
    ...
    return (
        <button 
            onClick={() => onSelectProfile(profile)}
            className="w-full bg-black/50 backdrop-blur-md rounded-xl p-4 border border-orange-500/20 flex items-center gap-4 text-left transition-all duration-300 hover:border-orange-500/40 hover:shadow-[0_0_25px_rgba(249,115,22,0.2)] hover:bg-zinc-900/50"
        >
