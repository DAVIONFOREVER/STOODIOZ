...
const ChooseProfile: React.FC<ChooseProfileProps> = ({ onSelectRole }) => {
    // Check URL parameters for pre-selected role (from invite links)
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (roleParam) {
                // Map URL param to UserRole
                const roleMap: Record<string, UserRole> = {
                    'STOODIO': UserRole.STOODIO,
                ...
                const role = roleMap[roleParam.toUpperCase()];
                if (role) {
                    // Auto-select the role after a brief delay to show the page
                    setTimeout(() => {
                        onSelectRole(role);
                    }, 500);
                }
        ...
        <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-1 gap-8">
                ...
                <RoleCard
                    ...
                    description="Scout talent, manage your roster's bookings, and oversee projects from a central dashboard."
