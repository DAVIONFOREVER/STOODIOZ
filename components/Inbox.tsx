                        ...
                        <li key={convo.id} onClick={() => onSelect(convo.id)}>
                            <div className={`p-4 flex items-center gap-4 cursor-pointer transition-colors duration-200 ${isSelected ? 'bg-orange-500/10' : 'hover:bg-zinc-800/50'}`}>
                                <div className="relative flex-shrink-0">
                                    <img loading="lazy" src={getProfileImageUrl(participant)} alt={participant.name} className="w-14 h-14 rounded-xl object-cover"/>
                        ...
                        {conversation.messages.map(msg => {
                            ...
                            return (
                                <div key={msg.id} className={`flex items-end gap-2 ${isUser ? 'justify-end' : 'justify-start'}`}>
                                    {!isUser && <img loading="lazy" src={getProfileImageUrl(participant)} className="w-6 h-6 rounded-full self-start"/>}
