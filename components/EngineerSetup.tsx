...
const EngineerSetup: React.FC<EngineerSetupProps> = ({ onCompleteSetup, onNavigate }) => {
    ...
    return (
        <div className="max-w-lg mx-auto bg-zinc-900/50 p-8 rounded-2xl border border-zinc-700/50 shadow-2xl">
            ...
            <h1 className="text-4xl font-extrabold text-center mb-2 text-slate-100">Create Your Engineer Profile</h1>
            <p className="text-center text-slate-400 mb-8">Showcase your skills and find your next session.</p>
            <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                    <label htmlFor="engineer-name" className="block text-sm font-medium text-slate-300 mb-2">Your Name</label>
                    <input type="text" id="engineer-name" value={name} onChange={e => setName(e.target.value)} className="w-full px-4 py-3 bg-zinc-800 border-zinc-700 rounded-lg" required />
                </div>
                <div>
                    <label htmlFor="engineer-bio" className="block text-sm font-medium text-slate-300 mb-2">Your Bio</label>
                    <textarea id="engineer-bio" value={bio} onChange={e => setBio(e.target.value)} rows={3} className="w-full px-4 py-3 bg-zinc-800 border-zinc-700 rounded-lg" placeholder="Describe your experience, specialties, and preferred genres."></textarea>
                </div>
                 <div>
