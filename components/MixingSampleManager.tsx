...
const MixingSampleManager: React.FC<MixingSampleManagerProps> = ({ engineer, onRefresh }) => {
    ...
    const handleSaveSample = async (sampleData: Omit<MixingSample, 'id' | 'audio_url' | 'engineer_id'>, audioFile: File | null) => {
        setIsUploading(true);
        const finalSample: Record<string, unknown> = {
            id: editingSample?.id || crypto.randomUUID(),
            ...sampleData,
            audio_url: editingSample?.audio_url || '',
            engineer_id: engineer.id,
            profile_id: engineer.id,
        };

        try {
            if (audioFile) {
                const row = await uploadMixingSampleFile(engineer.id, audioFile);
                (finalSample as any).audio_url = (row as any)?.url ?? (editingSample?.audio_url || '');
            }

            await upsertMixingSample(finalSample as any);
            onRefresh();
        } catch (error) {
