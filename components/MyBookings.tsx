...
import { CalendarIcon, ClockIcon, LocationIcon, RoadIcon, TrashIcon, DownloadIcon, MusicNoteIcon, BriefcaseIcon } from './icons';
                    ...
                    {bookings.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(booking => {
                        ...
                        <div key={booking.id} className={`bg-zinc-800 rounded-2xl shadow-lg p-6 flex flex-col md:flex-row gap-6 border border-zinc-700 hover:border-orange-500/50 transition-colors duration-300 ${booking.status === BookingStatus.CANCELLED ? 'opacity-60' : ''}`}>
                            <div className="flex-shrink-0">
                                ...
                                <img src={booking.stoodio?.image_url || booking.engineer?.image_url} alt={booking.stoodio?.name || booking.engineer?.name} className="w-full md:w-48 h-32 object-cover rounded-xl" />
                            </div>
                            ...
                            <div className="flex-shrink-0 flex md:flex-col items-center md:items-end justify-between gap-2">
                                 ...
                                 {booking.invoice_url && (
                                    <a 
                                        ...
                                        className="flex items-center gap-1.5 text-xs font-semibold text-zinc-400 hover:text-orange-400 transition-colors border border-zinc-600 px-3 py-1.5 rounded-md hover:border-orange-400"
                                    >
                                        <BriefcaseIcon className="w-4 h-4"/> Download Invoice
                                    </a>
                                 )}
