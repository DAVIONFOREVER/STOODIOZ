        ...
        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
                ...
                <div className="lg:col-span-3">
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 gap-4">
                        <div className="flex items-start gap-4">
                             <img src={engineer.image_url} alt={engineer.name} className="w-24 h-24 rounded-full object-cover border-4 border-zinc-700 -mt-12 shadow-lg" />
                             <div>
                                <h1 className="text-4xl font-extrabold text-orange-500">{engineer.name}</h1>
                                <div className="flex items-center gap-2 mt-1">
                                     <div className="flex items-center gap-1 text-yellow-400">
                                        <StarIcon className="w-4 h-4" />
                                        <span className="font-bold text-slate-200">{(engineer.rating_overall ?? 0).toFixed(1)}</span>
                                    </div>
                         ...
                         <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                            {canRequestMix && (
                                <button onClick={onOpenMixingModal} className="w-full sm:w-auto px-6 py-3 rounded-lg text-base font-bold transition-colors duration-200 flex items-center justify-center gap-2 shadow-md bg-indigo-500 text-white hover:bg-indigo-600">
                    ...
                    <p className="text-slate-300 leading-relaxed mb-8 mt-4">{engineer.bio}</p>
                ...
                <div className="lg:col-span-2 space-y-8">
                     <div className="p-6 cardSurface sticky top-28">
                        <h3 className="text-2xl font-bold mb-4 text-slate-100 flex items-center gap-2"><CalendarIcon className="w-6 h-6"/>Book Session</h3>
                        <p className="text-zinc-400 text-sm mb-4">Find a studio time and book {engineer.name} for an in-person session.</p>
                        <div className="space-y-4">
                            <div>
                                <label htmlFor="date" className="block text-sm font-medium text-slate-400 mb-1">Date</label>
                                <input type="date" id="date" value={date} onChange={e => setDate(e.target.value)} min={new Date().toISOString().split('T')[0]} className="w-full bg-zinc-900 border-zinc-700 rounded-lg p-2.5 text-white" />
                            </div>
                            <div>
                                <label htmlFor="time" className="block text-sm font-medium text-slate-400 mb-1">Time</label>
                                 <input type="time" id="time" value={time} onChange={e => setTime(e.target.value)} className="w-full bg-zinc-900 border-zinc-700 rounded-lg p-2.5 text-white" />
                            </div>
                            <button onClick={handleBookClick} disabled={!currentUser} className="w-full bg-orange-500 text-white font-bold py-3 rounded-lg hover:bg-orange-600 transition-all disabled:bg-slate-500 disabled:cursor-not-allowed shadow-lg">
                                Find a Stoodio
                            </button>
                        </div>
                    </div>
