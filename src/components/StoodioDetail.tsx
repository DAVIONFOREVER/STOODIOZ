...
const StoodioDetail: React.FC = () => {
    ...
    // Use local state to manage potentially hydrated studio object
    const [stoodio, setStoodio] = useState<Stoodio | null>(selectedStoodio);
    ...
    useEffect(() => {
        if (selectedStoodio?.id) {
            // Check if we need to hydrate missing data (like rooms)
            if (!selectedStoodio.rooms) {
                setIsLoadingDetails(true);
                fetchFullStoodio(selectedStoodio.id).then(fullData => {
                    if (fullData) {
                        setStoodio(fullData);
                        // Set default room if available once loaded
                        if (fullData.rooms && fullData.rooms.length > 0) {
                            setSelectedRoom(fullData.rooms[0]);
                        }
                    }
                    setIsLoadingDetails(false);
                });
            } else {
                setStoodio(selectedStoodio);
                if (selectedStoodio.rooms && selectedStoodio.rooms.length > 0) {
                     setSelectedRoom(selectedStoodio.rooms[0]);
                }
            }
            fetchUserPosts(selectedStoodio.id).then(setPosts);
        }
    }, [selectedStoodio]);
    ...
    // Safe access for following lists which might be missing in lightweight objects
    const following = stoodio.following || { artists: [], engineers: [], stoodioz: [], producers: [] };
                ...
                <div className="lg:col-span-2">
                    <div className="p-6 sticky top-28 cardSurface">
                        ...
                        <div className="space-y-4 mb-6">
                            {isLoadingDetails ? (
                                <p className="text-center text-zinc-500 animate-pulse">Loading rooms...</p>
                            ...
                            {!isLoadingDetails && (stoodio.rooms || []).length === 0 && <p className="text-center text-zinc-500">No rooms available.</p>}
                        </div>
