...
const Leaderboard: React.FC = () => {
    ...
    const rankedUsers = useMemo(() => {
        ...
        return filtered.sort((a, b) => {
            // Safely access properties with fallbacks to ensure algorithm inclusion
            const tierA = a.ranking_tier || RankingTier.Provisional;
            const tierB = b.ranking_tier || RankingTier.Provisional;
            
            const tierDiff = (tierOrder[tierB] || 0) - (tierOrder[tierA] || 0);
            if (tierDiff !== 0) return tierDiff;

            const sessionsA = a.sessions_completed || 0;
            const sessionsB = b.sessions_completed || 0;
            const sessionsDiff = sessionsB - sessionsA;
            if (sessionsDiff !== 0) return sessionsDiff;

            const ratingA = a.rating_overall || 0;
            const ratingB = b.rating_overall || 0;
            return ratingB - ratingA;
        });
    }, [activeRole, activeCity, artists, engineers, producers, stoodioz, tierOrder]);

    const handleProfileSelect = (profile: AllUsers) => {
        if ('amenities' in profile) viewStoodioDetails(profile as Stoodio);
        else if ('specialties' in profile) viewEngineerProfile(profile as Engineer);
