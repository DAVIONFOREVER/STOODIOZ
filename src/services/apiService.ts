...
export const getAllPublicUsers = async (): Promise<{
    ...
    const safeSelect = async (table: string, select: string) => {
        try {
            // PERFORMANCE: Fetch only essential data for the global list. 
            // Deep details (rooms, samples) are fetched via JIT functions below.
    ...
    const [artists, engineers, producers, stoodioz, labels] = await Promise.all([
        safeSelect('artists', '*'),
        safeSelect('engineers', '*'), // Lightweight
        safeSelect('producers', '*'), // Lightweight
        safeSelect('stoodioz', '*'),  // Lightweight
        safeSelect('labels', '*')
    ]);
...
// --- JUST-IN-TIME (JIT) FETCHERS ---
// These are called when a user clicks "View Profile" to ensure data integrity without bloating startup time.
...
export const fetchCurrentUserProfile = async (userId: string): Promise<{ user: any, role: UserRole } | null> => {
    ...
    try {
        ...
        if (profile && profile.role) {
            ...
            let table = '';
            // Load FULL details for the logged-in user immediately
            let query = '*'; 

            switch (role) {
                case UserRoleEnum.ARTIST: table = 'artists'; break;
                case UserRoleEnum.ENGINEER: table = 'engineers'; query = '*, mixing_samples(*)'; break;
                case UserRoleEnum.PRODUCER: table = 'producers'; query = '*, instrumentals(*)'; break;
                case UserRoleEnum.STOODIO: table = 'stoodioz'; query = '*, rooms(*), in_house_engineers(*)'; break;
                case UserRoleEnum.LABEL: table = 'labels'; break;
            }

            if (table) {
                const { data: userData } = await supabase.from(table).select(query).eq('id', userId).single();
                if (userData) return { user: userData, role };
            }
        }
    } catch (e) {
        console.warn("Fast profile lookup failed, falling back to table scan.");
    }

    // 2. SLOW PATH (Fallback): Check all tables (Necessary for legacy users before migration)
    const tables = [
    ...
    for (const t of tables) {
        try {
            ...
            if (data) {
                // Self-repair: Insert into profiles table for next time
...
export const createUser = async (userData: any, role: UserRole): Promise<Artist | Engineer | Stoodio | Producer | Label | { email_confirmation_required: boolean } | null> => {
    ...
    // Create the master profile entry for fast lookups
    try {
        await supabase.from('profiles').upsert({
            id: authUser.id,
            role: role,
        ...
        console.warn("Failed to create master profile entry", e);
    }
...
// ... (Keep existing implementation for other exports exactly as is)
...
export const fetchLabelContracts = async (labelId: string): Promise<LabelContract[]> => {
    ...
    const { data, error } = await supabase.from('label_contracts').select('*').eq('label_id', labelId);
    if (error) return [];
...
export const fetchActiveLabelContractForTalent = async (talentUserId: string): Promise<LabelContract | null> => {
    ...
    const { data, error } = await supabase.from('label_contracts').select('*').eq('talent_user_id', talentUserId).eq('status', 'active').single();
...
export const updateLabelContractRecoupBalance = async (contractId: string, amountToDeduct: number) => {
    ...
    const { data: contract } = await supabase.from('label_contracts').select('recoup_balance').eq('id', contractId).single();
    if (contract) {
        const newBalance = Math.max(0, contract.recoup_balance - amountToDeduct);
        await supabase.from('label_contracts').update({ recoup_balance: newBalance }).eq('id', contractId);
    }
};
...
const updateWallet = async (userId: string, table: string, amount: number, transaction: Transaction) => {
    ...
    const { data: user } = await supabase.from(table).select('wallet_balance, wallet_transactions').eq('id', userId).single();
    if (user) {
        const newBalance = (user.wallet_balance || 0) + amount;
        const newTransactions = [...(user.wallet_transactions || []), transaction];
        await supabase.from(table).update({ wallet_balance: newBalance, wallet_transactions: newTransactions }).eq('id', userId);
    }
};
...
export const getLabelBudgetOverview = async (labelId: string): Promise<LabelBudgetOverview | null> => {
    ...
    try {
        ...
        return { budget: budget || null, artists: artists };
    }
};
...
export const setLabelBudget = async (labelId: string, totalBudget: number) => {
    ...
    const { data, error } = await supabase.from('label_budgets').upsert({ label_id: labelId, total_budget: totalBudget, fiscal_year: currentYear }, { onConflict: 'label_id, fiscal_year' }).select().single();
    if (error) throw error;
    return data;
};
...
export const setArtistAllocation = async (labelId: string, artistId: string, allocation: number) => {
    const supabase = getSupabase();
    if (!supabase) return null;
    const { data, error } = await supabase.from('label_artist_budgets').upsert({ label_id: labelId, artist_id: artistId, allocation_amount: allocation }, { onConflict: 'label_id, artist_id' }).select().single();
    if (error) throw error;
    return data;
};
...
export const updateLabelBudgetMode = async (labelId: string, mode: LabelBudgetMode, monthlyAllowance?: number, resetDay?: number) => {
    ...
    if (!supabase) return null;
    const currentYear = new Date().getFullYear().toString();
    const { data, error } = await supabase.from('label_budgets').upsert({ label_id: labelId, budget_mode: mode, monthly_allowance: monthlyAllowance, reset_day: resetDay, fiscal_year: currentYear }, { onConflict: 'label_id, fiscal_year' }).select().single();
    if (error) throw error;
    return data;
};
...
export const addLabelFunds = async (labelId: string, amount: number, note: string) => {
    ...
    await supabase.from('label_budgets').upsert({ label_id: labelId, total_budget: newTotal, fiscal_year: currentYear }, { onConflict: 'label_id, fiscal_year' });
    const tx: Transaction = {
...
export const fetchLabelTransactions = async (labelId: string): Promise<Transaction[]> => {
    const supabase = getSupabase();
    if (!supabase) return [];
    const { data: labelUser, error } = await supabase.from('labels').select('wallet_transactions').eq('id', labelId).single();
...
export const toggleFollow = async (currentUser: any, targetUser: any, type: string, isFollowing: boolean) => {
    ...
    return { updatedCurrentUser: updatedCurrentUser || currentUser, updatedTargetUser: updatedTargetUser || targetUser };
};
...
export const acceptJob = async (booking: Booking, engineer: Engineer) => {
    const supabase = getSupabase();
    if(supabase) {
        const { data } = await supabase.from('bookings').update({ status: BookingStatus.CONFIRMED, engineer_id: engineer.id }).eq('id', booking.id).select().single();
...
export const endSession = async (booking: Booking) => {
    ...
    if(supabase) {
        ...
        if (talentId) {
            ...
            if (activeContract) {
                await supabase.from("notifications").insert({ recipient_id: activeContract.label_id, type: "LABEL_SESSION_COMPLETE" as any, message: `Session completed.`, read: false, timestamp: new Date().toISOString() });
            }
        }
...
export const fetchAnalyticsData = async (userId: string, role: UserRole, days: number): Promise<AnalyticsData> => {
    ...
    const { data: user } = await supabase.from(tableName).select('wallet_transactions').eq('id', userId).single();
    const transactions: Transaction[] = user?.wallet_transactions || [];
    const relevantTransactions = transactions.filter(t => new Date(t.date) >= startDate && t.amount > 0);
    const totalRevenue = relevantTransactions.reduce((sum, t) => sum + t.amount, 0);
    const revenueMap = new Map<string, number>();
    relevantTransactions.forEach(t => { const dateKey = t.date.split('T')[0]; revenueMap.set(dateKey, (revenueMap.get(dateKey) || 0) + t.amount); });
    const revenueOverTime = Array.from(revenueMap.entries()).map(([date, revenue]) => ({ date, revenue })).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    const sourceMap = new Map<string, number>();
    relevantTransactions.forEach(t => { const category = t.category || 'Other'; sourceMap.set(category, (sourceMap.get(category) || 0) + t.amount); });
    const revenueSources = Array.from(sourceMap.entries()).map(([name, revenue]) => ({ name, revenue }));
    const { count: bookingsCount } = await supabase.from('bookings').select('*', { count: 'exact', head: true }).or(`engineer_id.eq.${userId},producer_id.eq.${userId},stoodio_id.eq.${userId}`).gte('date', startDateStr).eq('status', BookingStatus.CONFIRMED);
    const { count: newFollowersCount } = await supabase.from('notifications').select('*', { count: 'exact', head: true }).eq('recipient_id', userId).eq('type', NotificationType.NEW_FOLLOWER).gte('timestamp', startDateStr);
    return { kpis: { totalRevenue, profileViews: 0, newFollowers: newFollowersCount || 0, bookings: bookingsCount || 0 }, revenueOverTime, engagementOverTime: [], revenueSources };
};
...
export const upsertRoom = async (room: Room, stoodioId: string) => {
    ...
    if(supabase) {
        ...
        const { data, error } = await supabase.from('rooms').upsert({ ...room, id: roomId, stoodio_id: stoodioId }).select().single();
        if(error) throw error;
        return data;
    }
};
...
export const upsertInHouseEngineer = async (info: InHouseEngineerInfo, stoodioId: string) => {
    const supabase = getSupabase();
    if(supabase) await supabase.from('in_house_engineers').upsert({ stoodio_id: stoodioId, engineer_id: info.engineer_id, pay_rate: info.pay_rate });
};
...
export const upsertInstrumental = async (inst: Instrumental, producerId: string) => {
    ...
    if(supabase) {
        ...
        await supabase.from('instrumentals').upsert({ ...inst, id: instId, producer_id: producerId });
    }
};
...
export const upsertMixingSample = async (sample: MixingSample, engineerId: string) => {
    ...
    if(supabase) {
        ...
        await supabase.from('mixing_samples').upsert({ ...sample, id: sampleId, engineer_id: engineerId });
    }
};
...
export const fetchLabelRoster = async (labelId: string): Promise<RosterMember[]> => {
    const supabase = getSupabase();
    if (!supabase) return [];
    const { data: rosterEntries } = await supabase.from('label_roster').select('*, artist:artists(*)').eq('label_id', labelId);
    const activityList = await getRosterActivity(labelId);
    const activityMap = Object.fromEntries((activityList || []).map((a: any) => [a.user_id, a]));
    const hydratedRoster: RosterMember[] = [];
    for (const entry of rosterEntries || []) {
        if (!entry.user_id) {
            hydratedRoster.push({ id: entry.id, name: entry.email || 'Pending Invite', email: entry.email, image_url: USER_SILHOUETTE_URL, role_in_label: entry.role, roster_id: entry.id, is_pending: true, followers: 0, follower_ids: [], following: { artists: [], engineers: [], producers: [], stoodioz: [], videographers: [], labels: [] }, wallet_balance: 0, wallet_transactions: [], coordinates: { lat: 0, lon: 0 }, show_on_map: false, is_online: false, rating_overall: 0, sessions_completed: 0, ranking_tier: 'Provisional' as any, is_on_streak: false, on_time_rate: 0, completion_rate: 0, repeat_hire_rate: 0, strength_tags: [], local_rank_text: '', claim_token: entry.claim_token });
            continue;
        }
        let userData = null;
        let shadowProfile = false;
        const { data: profile } = await supabase.from('profiles').select('role').eq('id', entry.user_id).single();
        if (profile && profile.role === 'UNCLAIMED') shadowProfile = true;
        ...
        if (userData) {
            ...
            hydratedRoster.push({ ...userData, role_in_label: entry.role, roster_id: entry.id, shadow_profile: shadowProfile, claim_token: entry.claim_token, sessions_completed: act.sessions_completed || userData.sessions_completed || 0, output_score: act.output_score || 0 });
        }
    }
    return hydratedRoster;
};

export const createShadowProfile = async (role: 'ARTIST' | 'PRODUCER' | 'ENGINEER', labelId: string, data: { name: string; email?: string }): Promise<{ profileId: string; roleId: string }> => {
    const supabase = getSupabase();
    if (!supabase) throw new Error("Supabase client not initialized");
    const shadowId = crypto.randomUUID();
    await supabase.from('profiles').insert({ id: shadowId, email: data.email || null, role: 'UNCLAIMED', full_name: data.name });
    const tableMap: Record<string, string> = { 'ARTIST': 'artists', 'PRODUCER': 'producers', 'ENGINEER': 'engineers' };
    const tableName = tableMap[role];
    const userRecord = { id: shadowId, name: data.name, email: data.email || null, image_url: USER_SILHOUETTE_URL, label_id: labelId, created_at: new Date().toISOString(), wallet_balance: 0, wallet_transactions: [] };
    ...
    await supabase.from(tableName).insert(userRecord);
    await supabase.from('label_roster').insert({ id: crypto.randomUUID(), label_id: labelId, user_id: shadowId, role: role.charAt(0).toUpperCase() + role.slice(1).toLowerCase(), email: data.email });
...
export const addArtistToLabelRoster = async (params: { labelId: string, artistId?: string, email?: string, role?: string }) => {
    ...
    const { data } = await supabase.from('label_roster').insert({ id: crypto.randomUUID(), label_id: labelId, user_id: artistId || null, email: email || null, role: role || 'Artist' }).select().single();
    if (artistId) await supabase.from('artists').update({ label_id: labelId }).eq('id', artistId);
    return data;
};
...
export const removeArtistFromLabelRoster = async (labelId: string, rosterId: string, artistId?: string) => {
    ...
    if (artistId) await supabase.from('artists').update({ label_id: null }).eq('id', artistId).eq('label_id', labelId);
    return true;
};
...
export const getClaimDetails = async (token: string): Promise<{ labelName: string; role: string; email?: string } | null> => {
    const supabase = getSupabase();
    if (!supabase) return null;
    const { data: rosterEntry } = await supabase.from('label_roster').select('*, label:labels(name)').or(`claim_token.eq.${token},claim_code.eq.${token}`).maybeSingle();
    if (!rosterEntry) return null;
    return { labelName: (rosterEntry.label as any)?.name || 'A Label', role: rosterEntry.role, email: rosterEntry.email };
};

export const claimProfileByToken = async (token: string, userId: string): Promise<{ role: UserRole }> => {
    const supabase = getSupabase();
    if (!supabase) throw new Error("Database not connected.");
    const { data: rosterEntry } = await supabase.from('label_roster').select('*').eq('claim_token', token).single();
    if (!rosterEntry) throw new Error("Invalid or expired claim token.");
    await supabase.from('label_roster').update({ user_id: userId, is_pending: false, claim_token: null, claim_code: null }).eq('id', rosterEntry.id);
    const role = rosterEntry.role.toUpperCase() as UserRole;
    let tableName = '';
    if(role === 'ARTIST') tableName = 'artists';
    if(role === 'ENGINEER') tableName = 'engineers';
    if(role === 'PRODUCER') tableName = 'producers';
    if (tableName) await supabase.from(tableName).update({ label_id: rosterEntry.label_id }).eq('id', userId);
    return { role };
};
...
export const claimProfileByCode = async (code: string, userId: string): Promise<{ role: UserRole }> => {
    const supabase = getSupabase();
    if (!supabase) throw new Error("Database not connected.");
    const { data: rosterEntry } = await supabase.from('label_roster').select('*').eq('claim_code', code).single();
    if (!rosterEntry) throw new Error("Invalid or expired claim code.");
    await supabase.from('label_roster').update({ user_id: userId, is_pending: false, claim_code: null, claim_token: null }).eq('id', rosterEntry.id);
    const role = rosterEntry.role.toUpperCase() as UserRole;
    let tableName = '';
    if(role === 'ARTIST') tableName = 'artists';
    if(role === 'ENGINEER') tableName = 'engineers';
...
export const claimLabelRosterProfile = async (details: { email: string; password: string; name?: string }): Promise<any> => {
    const supabase = getSupabase();
    if (!supabase) throw new Error("Database not connected.");
    const { data: rosterEntry } = await supabase.from('label_roster').select('*').eq('email', details.email).is('user_id', null).single();
    if (!rosterEntry) throw new Error("No pending invite found.");
    const { data: authData } = await (supabase.auth as any).signUp({ email: details.email, password: details.password, options: { data: { full_name: details.name || details.email, user_role: rosterEntry.role.toUpperCase() } } });
    await supabase.from('label_roster').update({ user_id: authData.user.id, is_pending: false }).eq('id', rosterEntry.id);
    const role = rosterEntry.role.toUpperCase() as UserRole;
    let tableName = '';
    if(role === 'ARTIST') tableName = 'artists';
    if(role === 'ENGINEER') tableName = 'engineers';
    if(role === 'PRODUCER') tableName = 'producers';
    if (tableName) {
         const profileData: any = { id: authData.user.id, name: details.name || details.email, email: details.email, image_url: USER_SILHOUETTE_URL, label_id: rosterEntry.label_id };
...
export const generateClaimTokenForRosterMember = async (rosterId: string): Promise<{ claimUrl: string }> => {
    ...
    await supabase.from('label_roster').update({ claim_token: token }).eq('id', rosterId);
...
export const getBookingEconomics = async (bookingId: string) => {
    ...
    return { total_cost: booking.total_cost, engineer_pay_rate: engineerPayRate, duration: booking.duration, contract, estimated_provider_take: engineerPayRate * booking.duration * (contract?.contract_type === "PERCENTAGE" ? (1 - contract.split_percent / 100) : 1), estimated_label_take: contract?.contract_type === "PERCENTAGE" ? engineerPayRate * booking.duration * (contract.split_percent / 100) : 0, recoup_remaining: contract?.recoup_balance || 0 };
};
