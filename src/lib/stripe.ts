...
const getStripe = (): Promise<any> => {
    if (!stripePromise) {
        ...
        const publishableKey = "pk_test_51SLl4UPQdrRi1Fr9g8FUPeKDK45boiPQmPCdpE9FGINBOyIUNsTMGr3DXNquPkgsPyx0Xr0I17szjcYPV7BRsn0e0040prQfo1";
        if (!publishableKey || !publishableKey.startsWith('pk_')) {
             console.error("Stripe publishable key is missing or invalid. Please set it in your environment variables.");
             return Promise.reject(new Error("Stripe key is not configured."));
        }
        stripePromise = new Promise((resolve, reject) => {
            if (typeof Stripe !== 'undefined') {
                ...
                reject(new Error("Stripe.js is not available."));
            }
        });
    }
...
export const redirectToCheckout = async (sessionId: string): Promise<void> => {
    try {
        ...
        // CRITICAL FIX: Re-throw the error so the calling function (hook) knows to stop the loading spinner.
        throw error;
    }
};
