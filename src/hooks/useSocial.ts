...
export const useSocial = () => {
    ...
    const toggleFollow = useCallback(async (type: 'stoodio' | 'engineer' | 'artist' | 'producer' | 'label', id: string) => {
        ...
        // Note: For labels viewing their roster, target might not be in the general lists yet if not fully hydrated
        // but for general follow actions it usually is.
        
        let isFollowing = false;
        // Determine if currently following based on user structure
        // Labels have a different structure sometimes, but here we assume standard 'following' object
        if ('following' in currentUser) {
             // Type guard for dynamic key access
             const key = `${type}s` as keyof typeof currentUser.following;
             isFollowing = (currentUser.following[key] || []).includes(id);
        }

        // Create optimistic copy of current user with updated following list
        ...
        try {
            if (!targetUser) {
                // If target not in global list, we might still want to try API (e.g. following a label)
                 // But for this hook, we usually need the object.
                 // Fallback: Just fire api call if we have ID.
            }
            
            // Pass the *original* isFollowing state to the API service so it knows what to toggle
            await apiService.toggleFollow(currentUser, { id } as any, type, isFollowing);

            // Note: We don't need to dispatch again if successful, unless we want to sync
            // updated follower counts on the target user.
