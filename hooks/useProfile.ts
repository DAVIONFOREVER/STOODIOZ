  ...
  const refreshCurrentUser = useCallback(async () => {
    ...
    try {
      ...
      const merged = {
        ...
        id: profileId,
        profile_id: profileId,
        name: (profRow?.display_name ?? profRow?.username ?? (resolvedRoleRow as any)?.stage_name ?? resolvedRoleRow?.name ?? profRow?.full_name ?? currentUser?.name) ?? null,
        display_name: profRow?.display_name ?? (resolvedRoleRow as any)?.display_name ?? null,
        username: profRow?.username ?? (resolvedRoleRow as any)?.username ?? null,
