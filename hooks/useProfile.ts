...
export const useProfile = () => {
    ...
    const updateProfile = async (
        updates: Partial<Artist | Engineer | Stoodio | Producer | Label>
    ) => {
        ...
        try {
            ...
            const updatedUsers = allUsers.map((u) =>
                u.id === fullUpdatedUser.id ? fullUpdatedUser : u
            );

            dispatch({
                type: ActionTypes.UPDATE_USERS,
                payload: { users: updatedUsers as any },
            });

            dispatch({
                type: ActionTypes.SET_CURRENT_USER,
                payload: { user: fullUpdatedUser },
            });
    ...
    const refreshCurrentUser = useCallback(async () => {
        ...
        try {
            ...
            if (data) {
                const refreshedUser =
                    data as unknown as Artist | Engineer | Stoodio | Producer | Label;
    ...
    const verificationSubmit = useCallback(
        async (
            ...
            try {
                const updatedStoodioPartial = await apiService.submitForVerification(
                    stoodioId,
                    data
                );

                const updatedUsers = allUsers.map((u) =>
                    u.id === stoodioId ? { ...u, ...updatedStoodioPartial } : u
                );
